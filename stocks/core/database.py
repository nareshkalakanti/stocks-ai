...
def init_db() -> None:
    with get_connection() as conn:
        conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS order_inflow_cache (
                ticker TEXT PRIMARY KEY,
                payload_json TEXT NOT NULL,
                fetched_at TEXT NOT NULL
            );

            CREATE INDEX IF NOT EXISTS idx_order_inflow_cache_fetched
                ON order_inflow_cache(fetched_at);
...
def save_order_inflow_cache(rows: list[dict]) -> None:
    if not rows:
        return
    init_db()
    now = _utc_now()
    with get_connection() as conn:
        for row in rows:
            ...
            if not ticker:
                continue
            payload = {"orders": row.get("orders") or []}
            conn.execute(
                """
                INSERT INTO order_inflow_cache (ticker, payload_json, fetched_at)
                VALUES (?, ?, ?)
                ON CONFLICT(ticker) DO UPDATE SET
                    payload_json=excluded.payload_json,
                    fetched_at=excluded.fetched_at
