"""Positive Surprise Quant — seasonal earnings surprise + PEG."""
...
def render_positive_surprise(*, show_title: bool = True) -> None:
    ...
    with scan_toolbar_row(*base_scan_extra_widths(COMPACT_SCAN_BTN_COL_WIDTH)) as row:
        filters, cap_tier_label_ui, holdings_industries_only = render_base_scan_filters(
            ...
            holdings_key="psq_holdings_industries_only",
        )
        ...
        with row[5]:
            run_clicked = st.button(
                ...
                key="psq_scan",
                help=(
                    f"Load PEAD fundamentals (cache ≤ {PEAD2_CACHE_HOURS}h), keep "
                    "positive seasonality-adjusted surprises, score with PEG overlay."
                ),
            )
    ...
    if run_clicked:
        ...
        progress = st.progress(0, text="Positive Surprise Quant — loading…")
        ...
        def _progress(done: int, total: int) -> None:
            ...
            progress.progress(
                min(done / total, 1.0),
                text=f"Positive Surprise Quant {done:,}/{total:,}…",
            )
        ...
        st.session_state.psq_cache_hits = int(result.get("cache_hits") or 0)
        coverage = result.get("coverage")
        if isinstance(coverage, Pead2ScanCoverage):
            st.session_state.psq_coverage = coverage
    ...
    candidates_previous = st.session_state.get("psq_candidates_previous")
    ...
    if not isinstance(candidates, pd.DataFrame) or candidates.empty:
        st.caption(
            "No positive seasonality-adjusted surprises in this universe "
            "(try widening filters or refreshing cache)."
        )
        return
    ...
    cache_hits = int(st.session_state.get("psq_cache_hits") or 0)
    ...
    embed_html = build_pead2_dashboard_html(
        candidates,
        df_previous=prev_df,
        title="Positive Surprise Quant",
        list_label="Positive surprises",
        standalone=False,
        variant="psq",
    ...
    st.caption(
        f"{len(candidates)} positive-surprise names · {cache_hits:,} from DB · "
        f"TQ **{tq_n}** · BB **{bb_n}** · "
    ...
    st.download_button(
        ...
        data=csv,
        file_name="positive_surprise_quant.csv",
        mime="text/csv",
        key="psq_csv",
    )
