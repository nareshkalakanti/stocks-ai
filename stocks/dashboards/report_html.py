...
_100X_STOCK_COLS: list[tuple[str, str, str | int]] = [
    ("sector", "Sector", "text"),
    ("criteria_score", "Score", 1),
    ("cfo_ebit_pct", "CFO/EBIT%", 1),
    ("ebt_capital_pct", "EBT/CE%", 1),
    ("cfo_mcap_pct", "CFO/MCap%", 1),
    ("cfo_latest_cr", "CFO Cr", 1),
    ("market_cap_cr", "Mkt cap Cr", 1),
    ("price", "Price", 2),
]
...
def build_100x_html(
    *,
    ...
    partial_count: int = 0,
    scanned: int = 0,
    title: str = "100X Formula — Pass",
    subtitle: str | None = None,
    standalone: bool = True,
) -> str:
    summary_body = (
        f'<div class="hr-method">'
        f"<strong>{len(pass_df)}</strong> pass (4/4 criteria) · "
        f"<strong>{partial_count:,}</strong> partial · "
        f"<strong>{scanned:,}</strong> scanned · "
        f"rising CFO · CFO/EBIT &gt; 60% · EBT/capital &gt; 12% · CFO/mcap &gt; 15%"
        f"</div>"
    )
    sections = (
        _metric_section(
            "100X Formula pass — 4/4 criteria",
            pass_df,
            metric_cols=_100X_STOCK_COLS,
            open_section=True,
        ),
        (
            f'<details class="fund-section" open>'
...
def formula_100x_iframe_height(row_count: int) -> int:
    return min(1200, max(480, 320 + min(row_count, 40) * 14))
