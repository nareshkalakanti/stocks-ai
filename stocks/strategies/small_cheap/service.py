"""Small + Cheap scan — yfinance mcap, P/S, and debt check."""
...
def prepare_small_cheap_universe(
    stocks: pd.DataFrame,
    *,
    ...
    if "market_cap_cr" in universe.columns:
        lo = SMALL_CHEAP_MCAP_MIN_CR
        hi = SMALL_CHEAP_MCAP_MAX_CR
...
def analyze_small_cheap_stock(
    ...
    *,
    ...
    if known_mcap_cr is not None and not in_small_cheap_mcap_band(known_mcap_cr):
        return None
    ...
    def _fetch() -> dict | None:
        ...
        if not in_small_cheap_mcap_band(market_cap_cr):
            return None
        ...
        metrics = compute_small_cheap_metrics(
...
def run_small_cheap_scan(
    ...
    *,
    ...
    workers = max_workers or yfinance_worker_count(
        len(universe), SMALL_CHEAP_MAX_WORKERS
    )
    ...
    for _, src in universe.iterrows():
        ...
        if known_mcap_f is not None and not in_small_cheap_mcap_band(known_mcap_f):
            continue
    ...
    scored = (
        score_small_cheap(raw, debt_free_only=debt_free_only)
        if not raw.empty
        else pd.DataFrame()
    )
    if not scored.empty:
