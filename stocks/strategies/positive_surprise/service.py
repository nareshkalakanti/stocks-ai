"""Positive Surprise Quant — scan orchestration on top of PEAD2 fundamentals."""
...
def run_positive_surprise_scan(
    ...
    *,
    ...
    """Fetch/score PEAD2 fundamentals, then re-rank on surprise + PEG."""
    ...
    scored = score_positive_surprise(candidates)
    ...
    if isinstance(prev, pd.DataFrame) and not prev.empty:
        prev_scored = score_positive_surprise(prev)
